...
const profile = {
  ...
  getters: {
    unSeenNotifications: (state) => {
      return state.notifications.filter((el) => el.is_view === false);
    },
  },
  ...
  actions: {
    ...
    seenNotifications({ commit, dispatch, rootState, state }) {
      let filteredNotifications;
      let unSeenIds = [];

      if (state.notifications.length > 0) {
        filteredNotifications = state.notifications.filter((el) => el.is_view === false);
      }

      filteredNotifications.map((el) => unSeenIds.push(el.id));
